    ...
    def __init__(self, 
        ...
        try:
            ...
            if self.canopen_handle is None:
                ...
                self.timeout_ms = self.settings.heartbeat_timeout_ms
                self.telemetry = Motor_Telemetry(self.node, self.Node_Name)
                # self.heartbeat_telemetry = self.telemetry.data.metadata.heartbeat
                # self.feedback = Feedback_Lib(self.node, self.Node_Name, self.canopen_handle, self.settings, self.telemetry)
                self.heartbeat = Heartbeat_Lib(self.node, self.Node_Name, self.canopen_handle, self.settings, self.telemetry, self.timeout_ms)
                ...
                self.log.print(f"Completed Loading settings for {node_name}","✅", "Motor_Heartbeat", "__init__")

        except Exception as e:
            self.log.print(f"Motor Motor_Heartbeat reading Setup Failure","❌","Motor_Heartbeat","__init__",f"{e}")
            self.log.print(f"❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌❌")
