...
from ros2_canbus.JS2_Motor_CANOpen_Lib_V_1_0.Motor_Mapping.data import statistics, StatuswordShared, errorcode, errorregister
...
class Motor_Telemetry:
    def __init__(self, node: canopen.RemoteNode, node_name: str):
        ...
        # self.telemetry = self.get_print_all_telemetry()
        # print("past telemetry class")

    # ---------------------------- SDO helpers ----------------------------
    def get_print_telemetry(self, telemetry_name: str, subindex: int = 0):
        """Read an OD entry (optionally a subindex), print it, return the raw value."""
        obj = self.node.sdo[telemetry_name]
        value = obj[subindex].raw if subindex else obj.raw
        self.log.print(value, "SDO", telemetry_name)
        self.common.delay_SDO()
        return value

    # ---------------------- High level poll + decode ---------------------
    def get_print_all_telemetry(self):
        try:
            # print("Passed here 5")
            ...
            # Feedback
            self.data.feedback.position = self.get_print_telemetry("Position_actual_value")
            self.data.feedback.velocity = self.get_print_telemetry("Velocity_actual_value")
            self.data.feedback.torque   = self.get_print_telemetry("Torque_actual_value")

            # Command
            self.data.command.position = self.data.feedback.position 
            self.data.command.velocity = 0
            self.data.command.torque   = 0

            # Settings (readbacks)
            self.data.settings.operationmode.read = self.get_print_telemetry("Modes_of_operation_display")
            self.data.settings.acceleration.read  = self.get_print_telemetry("Profile_acceleration")
            self.data.settings.deceleration.read  = self.get_print_telemetry("Profile_deceleration")
            self.data.settings.torque_slope.read  = self.get_print_telemetry("Torque_slope")
            self.data.settings.rated_current.read = self.get_print_telemetry("Motor_rated_current")

            # Meta
            md.current            = self.get_print_telemetry("Current_actual_value")
            md.voltage            = self.get_print_telemetry("Motor_bus_voltage")
            md.coiltemperature    = self.get_print_telemetry("Coil_temperature_value")
            md.circuittemperature = self.get_print_telemetry("Circuit_board_temperature_value")
            md.statusword.raw     = self.get_print_telemetry("Statusword")
            md.errorregister.raw  = self.get_print_telemetry("Error_register")
            md.errorcode.raw      = self.get_print_telemetry("Error_code")
            md.heartbeat.interval = self.get_print_telemetry("Producer_heartbeat_time")
            md.heartbeat.state    = Avatarrobot_CANopen_Map.HeartbeatState.UNKNOWN
            md.heartbeat.count    = 0
            md.state              = None

            # Decode 0x1001 + 0x603F
            md.errorregister, md.errorcode = self.parse_and_flood_error_register_error_code(
                md.errorregister.raw,
                md.errorcode.raw
            )
            self.print_errorregister(md.errorregister)
            self.print_errorcode(md.errorcode)


            # Decode 0x6041 + state (uses mode 0x6061)
            # sw, st = 
            # print("Passed here 3")
            md.statusword = self.parse_statusword_state(md.statusword.raw, self.data.settings.operationmode.read)
            # print("Passed here 4")
            ...
            self.log.print(f"Initial telemetry Failure","❌","get_print_all_telemetry","Motor_Telemetry",f"{e}")
